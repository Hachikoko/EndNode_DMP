...
extern void BspFixToDou(u8 ucType, u8 ucInteger, u8 ucdecimal, u32 llfix, double *pdbRet);
...
static void read_from_mpl(void)
{
	...
	double data_result[4];	
	char buf[300];
	int n;
	int i = 0;
	
	NODE_DATA temp;
	NODE_DATA* p_temp = NULL;
	
	
	if (inv_get_sensor_type_accel(data, &accuracy,(inv_time_t*)&timestamp)){
		memset(data_result,0,sizeof(double)*4);
		BspFixToDou(1,15,16,data[0],&data_result[0]);
		BspFixToDou(1,15,16,data[1],&data_result[1]);
		BspFixToDou(1,15,16,data[2],&data_result[2]);
		memset(data,0,sizeof(long)*4);
		
		temp.acc_x = (short)(data_result[0]*10000.0f);
		temp.acc_y = (short)(data_result[1]*10000.0f);
		temp.acc_z = (short)(data_result[2]*10000.0f);
	...
	if (inv_get_sensor_type_gyro(data, &accuracy,(inv_time_t*)&timestamp)){
		...
		temp.gyr_x = (short)(data_result[0]*10.0f);
		temp.gyr_y = (short)(data_result[1]*10.0f);
		temp.gyr_z = (short)(data_result[2]*10.0f);
	...
	if (inv_get_sensor_type_compass(data, &accuracy,(inv_time_t*)&timestamp)){
		...
		temp.mag_x = (short)(data_result[0]*100.0f);
		temp.mag_y = (short)(data_result[1]*100.0f);
		temp.mag_z = (short)(data_result[2]*100.0f);
	...
	if (inv_get_sensor_type_linear_acceleration(float_data, &accuracy,(inv_time_t*)&timestamp)){
		...
		temp.acc_line_x = (short)(float_data[0]*100.0f);
		temp.acc_line_y = (short)(float_data[1]*100.0f);
		temp.acc_line_z = (short)(float_data[2]*100.0f);
		
	}
	
	if (inv_get_sensor_type_quat(data, &accuracy, (inv_time_t*)&timestamp)) {
		memset(data_result,0,sizeof(double)*4);
		BspFixToDou(1,1,30,data[0],&data_result[0]);
		BspFixToDou(1,1,30,data[1],&data_result[1]);
		BspFixToDou(1,1,30,data[2],&data_result[2]);
		BspFixToDou(1,1,30,data[3],&data_result[3]);
		memset(data,0,sizeof(long)*4);
		
		temp.q0 = (short)(data_result[0]*10000.0f);
		temp.q1 = (short)(data_result[1]*10000.0f);
		temp.q2 = (short)(data_result[2]*10000.0f);
		temp.q3 = (short)(data_result[3]*10000.0f);
	}
	
	push_back_node(ptr_link_queue,&temp);
	
//	if(ptr_link_queue->length > 2){
//		Uart_send('c');
//		p_temp = &ptr_link_queue->front->next->data;
//	
//		sprintf(buf,"acc_x:%d,acc_y:%d,acc_z:%d,gry_x:%d,gry_y:%d,gry_z:%d,mag_x:%d,mag_y:%d,mag_z:%d\r\n q0:%d,q1:%d,q2:%d,q3:%d\r\n",p_temp->acc_x,p_temp->acc_y,p_temp->acc_z,p_temp->gyr_x,p_temp->gyr_y,p_temp->gyr_z
//			,p_temp->mag_x,p_temp->mag_y,p_temp->mag_z,p_temp->q0,p_temp->q1,p_temp->q2,p_temp->q3);
//		n = strlen(buf);
//		for(i = 0; i < n; i++){
//			Uart_send(buf[i]);
//		}
